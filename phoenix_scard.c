...
static int prt_set_ctrl(int fd, const int  ctrl)
{
  int tmp;
...
//      return ;
 ...
 if (0) {
  ...
  printf("RESET\n");
  prt_set_ctrl(fd,TIOCM_RTS | TIOCM_CTS | TIOCM_DTR);
  sleep(1);
  prt_set_ctrl(fd,TIOCM_CTS | TIOCM_DTR);
   usleep(200);
...
s->blen = 0;
//printf("reading %d bytes in %d ms...\n",len,ta);
...
void *f = s->f;
if (s->logLevel>5) hexdump("APDU sending:",apdu,len);
...
if(s->logLevel>3) printf("==>now - wait CLA %x\n",cla);
sc_read(s,1,2000);
if ( (s->blen == 1) && (s->buf[0]==cla))  {
   if (s->logLevel>3) printf("command OK, send a rest %d!\n",len-5);
  } else {
...
// 0x60 - wait next byte ....
sc_read(s, 2+expected,s->taCommand); //  status + expected bytes
return s->blen>=2;
}
...
s->f = f; s->taCommand = 1000; // one second maximum wait for command timeout
